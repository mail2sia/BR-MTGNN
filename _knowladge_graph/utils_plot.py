...
def parse_args():
    ...
    p.add_argument("--node-spacing", type=float, default=0.5,
                   help="Node spacing factor (passed to spring_layout as k). Increase to spread nodes.")
    p.add_argument("--dpi", type=int, default=300, help="Figure DPI / resolution")
...
def compute_layout(G: nx.Graph, args):
    # Use spring layout by default; respect node-spacing (k) for spread
    # nx.spring_layout's `k` controls optimal distance between nodes.
    try:
        k = float(args.node_spacing)
    except Exception:
        k = None
    return nx.spring_layout(G, seed=args.seed, k=k)
