"""
=============================================================================
PIPELINE STEP 3 of 5 — train_test.py
=============================================================================
Purpose : Validation training run with chronological train/val/test split
          (70% / 20% / 10%). Trains on train set, evaluates on val set,
          saves the best validation checkpoint and model configuration.

Run AFTER: make_sparse_graph.py  (Step 2) — requires:
               data/sm_data.csv
               data/graph_sparse.csv
Run BEFORE: train.py  (Step 4)
...
----------
    ...
    python train_test.py \\
        ...
        --use_tdb_input \\
        --epochs 250 --patience 35

Key options:
    --device        cuda:1 (GPU) or cpu
    --seq_in_len    Input context window in months (default 10)
    --seq_out_len   Forecast horizon in months (default 36)
    --use_tdb_input Train on combined NoM+NoP signal (recommended)
    --epochs        Max training epochs (default 250)
    --patience      Early stopping patience (default 35)
    --batch_size    Mini-batch size (default 16; reduce to 8 if OOM)

Outputs:
    model/Bayesian/model.pt           Best validation epoch weights
    model/Bayesian/hp.txt             Model configuration (JSON)
    model/Bayesian/metadata.json      Data schema, scalers, conformal quantiles
    model/Bayesian/training_history.csv  Per-epoch train/val metrics

Expected time: ~5 min on GPU (cuda:1)
=============================================================================
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="BC-MTGNN training/testing for RMD NoM, RMD NoP, and PT NoM forecasting")
    ...
    p.add_argument("--train_end_date", type=str, default="",
                   help="Hard date cutoff for training targets (YYYY-MM). "
                        "When set with --val_end_date, overrides ratio-based splitting "
                        "to prevent target-period leakage. Example: 2018-09")
    p.add_argument("--val_end_date", type=str, default="",
                   help="Hard date cutoff for validation targets (YYYY-MM). "
                        "Targets after this date go to test. Example: 2021-09")
...
def main():
    ...
    if args.train_end_date and args.val_end_date:
        print(f"Split mode: DATE-BASED (train≤{args.train_end_date}, val≤{args.val_end_date}, test=remainder)")
    else:
        print(f"Split mode: RATIO-BASED ({args.train_ratio}/{args.valid_ratio}) — consider --train_end_date/--val_end_date for publication")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        valid_loss = eval_loss(model, valid_loader, data, args, device)
        ...
        history.append({"epoch": epoch, "train_loss": train_loss, "valid_loss": valid_loss, "lr": current_lr})

        print(f"| epoch {epoch:03d} | train_loss {train_loss:.6f} | valid_loss {valid_loss:.6f} | lr {current_lr:.2e}")
    ...
    hp["_note"] = "Fixed model configuration selected by manual tuning; not the result of an automated search."
