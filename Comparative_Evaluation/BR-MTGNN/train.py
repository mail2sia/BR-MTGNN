...
Workflow:
  - Loads model/Bayesian/model.pt from train_test.py (or best_model.pt for backward compatibility)
  - Trains on 100% of data for 500 epochs
  - Applies forecast-continuity regularization to prevent unrealistic spikes
  - Saves model/Bayesian/o_model.pt as the final production model

Continuity regularization (prevents forecast divergence):
  1. lambda_delta    : weak global anchoring to recent level
  2. lambda_start    : first forecast step should not jump far from last observed input
  3. lambda_horizon  : forecast horizon should avoid artificial month-to-month steps

Output:
  - model/Bayesian/o_model.pt                (final production model, ready for forecast.py)
  - model/Bayesian/production_train_metadata.json (training metadata)
  - model/Bayesian/hp.txt                    (saved hyperparameters)

Usage:
    python train.py --device cuda:0 --output_dir model/Bayesian
"""
...
def load_checkpoint_compat(path: Path, device: torch.device):
    """
    Load legacy or state-dict checkpoints across PyTorch versions.
...
def parse_args():
    p = argparse.ArgumentParser(description="BR-MTGNN full-data final training")
...
def main():
    ...
    try:
        ...
        if getattr(args, "use_best_model_init", True) and init_path:
            ...
            state = load_checkpoint_compat(init_path, device)
            loaded = False
    ...
    save_metrics_table(metrics_dir / "production_metrics.csv", "production", train_metrics)
