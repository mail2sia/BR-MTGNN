...
def evaluate_sliding_window(data, test_window, model, evaluateL2, evaluateL1, n_input, is_plot):
    ...
    scale = data.scale.to(device).expand(test_window.size(0), data.m) #scale will have the max of each column (142 max values)
...
def evaluate(data, X, Y, model, evaluateL2, evaluateL1, batch_size, is_plot):
    ...
    for X, Y in data.get_batches(X, Y, batch_size, False):
        ...
        scale = data.scale.to(device).expand(Y.size(0), Y.size(1), data.m) #scale will have the max of each column (142 max values)
        y_pred_o=output
        y_true_o=Y
...
def train(data, X, Y, model, criterion, optim, batch_size):
    ...
    for X, Y in data.get_batches(X, Y, batch_size, True):
        ...
        for j in range(args.num_split):
            ...
            scale = data.scale.to(device).expand(output.size(0), output.size(1), data.m)
...
parser.add_argument('--data', type=str, default='./data/sm_data.csv',
...
parser.add_argument('--lr_schedule',type=str,default='none',choices=['none','plateau','cosine'],help='learning rate schedule')
parser.add_argument('--lr_patience_sched',type=int,default=10,help='patience for ReduceLROnPlateau scheduler')
...
def main(experiment):
    ...
    #random search
    for q in range(1):  # Only 1 trial (hyperparameter search disabled for fast baseline comparison)
        ...
        if args.lr_schedule == 'plateau':
            scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
                optim.optimizer if hasattr(optim, 'optimizer') else optim, mode='min',
                factor=args.lr_decay_factor, patience=args.lr_patience_sched, min_lr=args.lr_min
            )
        elif args.lr_schedule == 'cosine':
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                optim.optimizer if hasattr(optim, 'optimizer') else optim, T_max=args.epochs, eta_min=args.lr_min
            )
        else:
            scheduler = None
        ...
        try:
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                if bad_epochs >= args.patience:
                    print(f'Early stopping at epoch {epoch}; best validation loss={best_valid:.6f}')
                    break
