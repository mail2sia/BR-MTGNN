...
Quick start:
    ...
    python ./scripts/train_test.py --device cuda:0 --train --trainer_mode --epochs 300 \
        --has_header --residual_head --temporal_attn
    
    # Evaluation only (uses existing checkpoint)
    python ./scripts/train_test.py --device cuda:0 --eval_only --has_header
    
    # Override specific parameters
    python ./scripts/train_test.py --device cuda:0 --lr 0.0001 --batch_size 16 --train
    
    # Load from metrics JSON (reproduce exact run)
    python ./scripts/train_test.py --metrics_json <path>/metrics_validation.json --train
...
def _get_plot_out_dir(kind: str) -> str:
    return os.path.join('model', str(kind))


def plot_predicted_actual(pred, true, title, kind, ci=None, base_year=None, steps_per_year=None, base_month=1):
    """Generate and save a plot of predicted vs. actual values (example code style)."""
    try:
        ...
        if ci_np is not None and len(ci_np) == len(pred_np):
            plt.fill_between(x, pred_np - ci_np, pred_np + ci_np,
        ...
        # Assumption: data is at monthly frequency (steps_per_year = 12 months/year)
        if base_year is not None and steps_per_year is not None and steps_per_year > 0:
            try:
                ...
                sty = int(steps_per_year)
                
                tick_pos = []
                tick_labels = []
                
                # For monthly data with steps_per_year=12:
                # Each data point represents one month
                for i in range(len(pred_np)):
                    # Calculate absolute month from start
                    total_months_from_base = i + (bm - 1)
                    # Calculate year and month
                    year_num = by + (total_months_from_base // 12)
                    month_num = (total_months_from_base % 12) + 1
                    
                    # Semiannual markers: Jun(06), Dec(12) to reduce tick crowding
        ...
        plt.savefig(os.path.join(out_dir, f"{title_fs}_{kind}.pdf"), bbox_inches="tight", format='pdf')
...
def save_metrics_1d(predict, test, title, kind):
    ...
    out_dir = _get_plot_out_dir(kind); os.makedirs(out_dir, exist_ok=True)
...
def evaluate_sliding_window(data, test_window, model, evaluateL2, evaluateL1, n_input, is_plot, mc_runs=None):
    ...
    if is_plot:
        ...
        is_chrono = bool(getattr(args, 'use_chronological_split', False)) or bool(getattr(args, 'chronological_split', False))
        ...
        if is_chrono:
            ...
            # Ratio-based split: calculate dates from parsed data
            ...
            if data_start_year is not None and data_start_month is not None:
                ...
                if idx_set and len(idx_set) > 0:
                    ...
                    print(f"[DateMapping] Testing split starts at {base_year_test:04d}-{base_month_test:02d} (window index {first_window_idx})")
        
        # Apply offset for chronological splits
        if base_year_test is not None and is_chrono:
            try:
                idx_set = getattr(data, '_test_idx_set', None)
                if idx_set:
                    base_year_test, base_month_test = _offset_year_month(base_year_test, base_month_test, int(idx_set[0]), steps_per_year)
            except Exception:
                pass
        ...
        for col in range(n_cols):
            ...
            # Only plot RMD series
            if 'RMD' in node_name:
                save_metrics_1d(torch.from_numpy(po[:,col]), torch.from_numpy(yo[:,col]), node_name, 'Testing')
                plot_predicted_actual(po[:,col], yo[:,col], node_name, 'Testing', ci=co[:,col], base_year=base_year_test, steps_per_year=steps_per_year, base_month=base_month_test)
...
def evaluate(data, X, Y, model, evaluateL2, evaluateL1, batch_size, is_plot, mc_runs=None, kind='Validation'):
    ...
    if is_plot:
        ...
        # Determine base year/month for x-axis labeling
        # Works for both chronological and ratio-based splits
        is_chrono = bool(getattr(args, 'use_chronological_split', False)) or bool(getattr(args, 'chronological_split', False))
        steps_per_year = int(getattr(args, 'steps_per_year', 12))
        base_year = None
        base_month = 1
        ...
        if is_chrono:
            # Chronological split: use configured year boundaries
            base_year = (args.train_end_year + 1) if kind == 'Validation' else (args.valid_end_year + 1)
            base_month = 1
        else:
            # Ratio-based split: calculate dates from parsed data
            # Get starting dates from data object if available
            data_start_year = getattr(data, 'start_year', None)
            data_start_month = getattr(data, 'start_month', None)
            
            if data_start_year is not None and data_start_month is not None:
                # Get the index set for this split to know where it starts in the data
                idx_set = getattr(data, '_valid_idx_set', None) if kind == 'Validation' else getattr(data, '_test_idx_set', None)
                
                if idx_set and len(idx_set) > 0:
                    # Calculate the starting date for this split
                    # idx_set[0] is the index of the first window in this split
                    first_window_idx = int(idx_set[0])
                    
                    # Calculate total months from data start to this split
                    # Each index represents a window, and for monthly data, indices map to months
                    total_months_from_start = first_window_idx
                    ...
                    print(f"[DateMapping] {kind} split starts at {base_year:04d}-{base_month:02d} (window index {first_window_idx})")
                else:
                    print(f"[DateMapping] Warning: Could not find index set for {kind} split")
            else:
                print(f"[DateMapping] Warning: No date information found in data object")
        
        # Apply offset if index_set available for chronological splits
        if base_year is not None and is_chrono:
            try:
                idx_set = getattr(data, '_valid_idx_set', None) if kind == 'Validation' else getattr(data, '_test_idx_set', None)
                if idx_set:
                    base_year, base_month = _offset_year_month(base_year, base_month, int(idx_set[0]), steps_per_year)
            except Exception:
                pass
...
def train(data, X, Y, model, criterion, optim, batch_size,
    ...
    # DISABLED: Computation was unused in loss function
    # try:
    #     with torch.no_grad():
    ...
    # except Exception:
    #     node_weights = None
    node_weights = None  # Explicitly disabled
        ...
        if use_amp:
            ...
            if getattr(args, 'use_weighted_horizon_loss', False):
                ...
                if getattr(args, 'residual_head', False):
                    ...
                    # NOTE: y_o is already absolute future value, only add baseline to prediction
                ...
                if getattr(args, 'residual_head', False):
                    ...
                    out_o = out_o + baseline_o
                    # NOTE: y_o is already absolute future value, only add baseline to prediction
                # 3. Calculate shape-based loss (sMAPE) in original units
...
parser.add_argument('--normalize', type=int, default=3, help='Normalization mode: 3=per-node z-score (recommended for heterogeneous nodes)')
...
parser.add_argument('--dropout',type=float,default=0.0,help='dropout rate (0.0 recommended for stable training)')
...
parser.add_argument('--conv_channels',type=int,default=96,help='convolution channels (96 for optimized config)')
...
parser.add_argument('--residual_channels',type=int,default=96,help='residual channels (96 for optimized config)')
parser.add_argument('--skip_channels',type=int,default=192,help='skip channels (192 for optimized config)')
parser.add_argument('--end_channels',type=int,default=384,help='end channels (384 for optimized config)')
...
parser.add_argument('--seq_in_len',type=int,default=24,help='input sequence length (24 months = 2 years context)')
parser.add_argument('--seq_out_len',type=int,default=36,help='output sequence length (36 months = 3 years forecast)')
parser.add_argument('--horizon', type=int, default=1) 
parser.add_argument('--layers',type=int,default=5,help='number of layers')

parser.add_argument('--batch_size',type=int,default=8,help='batch size (8 for more stable gradients)')
parser.add_argument('--lr',type=float,default=0.0005,help='learning rate (0.0005 for stable convergence)')
parser.add_argument('--weight_decay',type=float,default=0.00001,help='weight decay rate')

parser.add_argument('--clip', type=float, default=30.0, help='Gradient norm clip (30.0 for temporal attention stability)')
...
parser.add_argument('--search_trials', type=int, default=30,
...
parser.add_argument('--train_ratio', type=float, default=0.60,
                    help='Train ratio for ratio-based chronological split of historical data (2004-2025). For 21 years: 60%=~12.6y train, 20%=~4.2y val, 20%=~4.2y test. Future forecasting (2026-2028) uses forecast.py.')
...
parser.add_argument('--y_transform', type=str, default='none', choices=['none','log1p'],
...
parser.add_argument('--trend_smooth', action='store_true',
                    help='Enable trend-aware smoothing (Holt trend + smoothed residual).')
parser.add_argument('--trend_alpha', type=float, default=0.25,
                    help='Holt level smoothing alpha (0-1).')
parser.add_argument('--trend_beta', type=float, default=0.05,
                    help='Holt trend smoothing beta (0-1).')
parser.add_argument('--resid_alpha', type=float, default=0.20,
...
parser.add_argument('--horizon_gamma', type=float, default=1.0,
...
parser.add_argument('--residual_head', action='store_true', help='Train/predict residuals relative to last input level per-sample (original units). RECOMMENDED for better amplitude tracking.')
# Dual-channel input option: none/diff/pct (must be defined BEFORE parse_args)
parser.add_argument('--dual_channel', choices=['none', 'diff', 'pct'], default='pct',
                    help='Second input channel: pct (percentage changes, scale-invariant, RECOMMENDED), diff (z[t]-z[t-1]), or none for single-channel')
parser.add_argument('--pct_clip', type=float, default=3.0,
...
parser.add_argument('--amp_loss_weight', type=float, default=0.0,
...
parser.add_argument('--smooth_actual_alpha', type=float, default=None,
                    help='Smoothing alpha for actual curve (defaults to max(smooth_alpha, 1.5*smooth_alpha capped at 0.6))')
parser.add_argument('--smooth_actual_passes', type=int, default=1,
...
parser.add_argument('--despike_plots', action='store_true', default=False,
                    help='Remove noise spikes from actual curves in plots (median filter only, no clipping)')
parser.add_argument('--despike_median_window', type=int, default=5,
                    help='Median filter window size for spike removal in plots')
parser.add_argument('--plot_trend_smooth', action='store_true',
                    help='Use trend-aware smoothing for actual curves in plots (Holt trend + residual EMA)')
parser.add_argument('--plot_trend_alpha', type=float, default=0.25,
                    help='Holt level alpha for plot smoothing (0-1)')
parser.add_argument('--plot_trend_beta', type=float, default=0.05,
                    help='Holt trend beta for plot smoothing (0-1)')
parser.add_argument('--plot_resid_alpha', type=float, default=0.20,
                    help='Residual EMA alpha for plot smoothing (0-1)')
parser.add_argument('--plot_trend_clamp_min', type=float, default=None,
                    help='Clamp minimum before plot trend smoothing (use None to allow negatives)')
parser.add_argument('--plot_trend_impute', action='store_true',
                    help='Impute spike picks to trend (robust residual outlier detection)')
parser.add_argument('--plot_trend_impute_window', type=int, default=5,
                    help='Window size for robust spike detection (odd >=3)')
parser.add_argument('--plot_trend_impute_k', type=float, default=3.0,
...
parser.add_argument('--loss_alpha', type=float, default=0.5, help='Weight of L1 term (0.5 for MAE-dominant)')
parser.add_argument('--loss_beta',  type=float, default=0.0, help='Weight of 1-corr term (0.0 disables correlation penalty which can cause lag)')
parser.add_argument('--loss_gamma', type=float, default=0.5, help='Weight of sMAPE term (0.5 for balanced contribution)')
parser.add_argument('--mae_weight', type=float, default=10.0, help='Extra MAE (original units) - 10.0 makes MAE ~90.9% of total loss')
...
parser.add_argument('--sched_T0', type=int, default=15, help='CosineWarmRestarts T0 (15 for optimized config)')
...
parser.add_argument('--attn_heads', type=int, default=4,
                    help='Number of attention heads for temporal MHSA (4 for optimized config).')
parser.add_argument('--attn_dim', type=int, default=96,
                    help='Projection dimension for temporal MHSA (Q/K/V) - 96 for optimized config.')
parser.add_argument('--attn_dropout', type=float, default=0.0,
                    help='Dropout inside temporal MHSA block (0.0 for stable training).')
...
parser.add_argument('--temporal_transformer', type=int, default=1,
                    help='Enable Transformer encoder over time (1=enabled, RECOMMENDED).')
parser.add_argument('--tt_layers', type=int, default=2,
                    help='Number of Transformer encoder layers when enabled (2 recommended).')

# --- Graph fusion + DropEdge -------------------------------------------------
parser.add_argument('--graph_mix', type=float, default=0.5,
                    help='Blend ratio alpha for predefined vs learned adjacency (0.5 = equal mix, RECOMMENDED).')
parser.add_argument('--dropedge_p', type=float, default=0.08,
...
parser.add_argument('--compile', type=str, choices=['off','eager','inductor','auto'], default='off',
...
if getattr(args, "runlog", False):
    ...
    import torch
    if os.environ.get('BMTGNN_DEBUG_DEVICE', '0') == '1':
        print(f"[RunLog] Logging to: model/Bayesian/logs/...")
    devprops = {}
...
def main(experiment): # pyright: ignore[reportGeneralTypeIssues]
    ...
    # Set custom process title with Shakil's name and device info
    if setproctitle is not None:
        try:
            device_str = str(args.device).replace(':', '_')
            epochs = getattr(args, 'epochs', 100)
            proc_title = f'Shakil_natogpu_{device_str}_ep{epochs}'
            setproctitle.setproctitle(proc_title)
        except Exception as e:
            pass  # Silent process title failure
    ...
    # Backward compatibility: historically the default graph path pointed to top-k.
    # If the user did not override it, prefer the square adjacency by default.
    if args.graph == './data/graph_topk_k12.csv':
        args.graph = './data/graph_square.csv'  # Use square adjacency by default
    ...
    # If num_nodes still unset (0 or negative), adopt from loaded data BEFORE graph loading
    if getattr(args, 'num_nodes', 0) <= 0:
        try:
    ...
    if run_search:
        ...
        if device.type == 'cuda':
            ...
            print(f"[cleanup] Initial GPU memory cleared before search")
    ...
    if getattr(args, 'fresh_start', False):
        ...
        model = gtnet(
            gcn_true=args.gcn_true, buildA_true=args.buildA_true,
            gcn_depth=args.gcn_depth, num_nodes=args.num_nodes,
            device=device, predefined_A=predefined_A,
            dropout=args.dropout, subgraph_size=args.subgraph_size,
            node_dim=args.node_dim, dilation_exponential=args.dilation_exponential,
            conv_channels=args.conv_channels, residual_channels=args.residual_channels,
            skip_channels=args.skip_channels, end_channels=args.end_channels,
            seq_length=args.seq_in_len, in_dim=in_dim_actual,
            out_dim=args.seq_out_len, layers=args.layers,
            ...
            if not loaded_hp:
                ...
                if os.environ.get('BMTGNN_SKIP_CKPT_INFER', '0') == '1':
                    ...
                    try:
                        ...
                        print('[info] Using seq_in_len from command-line args:', getattr(args,'seq_in_len', None))
                    except Exception as _e:
    ...
    # Reconstruct Data using ratio-based splitting (chronological split removed)
    Data = DataLoaderS(args.data, float(getattr(args,'train_ratio',0.6)), float(getattr(args,'valid_ratio',0.2)), device, args.horizon, args.seq_in_len, args.normalize, args.seq_out_len,
                       chronological=False,
                       dual_channel=args.dual_channel,
                       pct_clip=float(getattr(args, 'pct_clip', 0.0)),
